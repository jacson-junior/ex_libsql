defmodule ExLibSQL.Pragma do
  @moduledoc """
  Handles parsing extra options for the libSQL connection
  """
  ...
  def journal_mode(options) do
    case Keyword.get(options, :journal_mode, :wal) do
      :delete -> "delete"
      :memory -> "memory"
      :off -> "off"
      :persist -> "persist"
  ...
  def temp_store(options) do
    case Keyword.get(options, :temp_store, :memory) do
      :file -> 1
      :memory -> 2
      :default -> 0
